...
export default function EducationHub({ onBack, user }) {
  ...
  return (
    <div style={{ padding: "24px 28px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <button onClick={() => setSubTab("budget")} style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontFamily: MN, fontSize: 14, fontWeight: 700, color: C.accent }}>Controle de Gastos</span>
            <span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${C.accent}15`, color: C.accent, fontFamily: MN }}>Disponível</span>
          </div>
          <p style={{ fontSize: 12, color: C.textDim, margin: "4px 0 0", lineHeight: 1.6 }}>Controle despesas variáveis, defina limites mensais e acompanhe seus gastos em tempo real.</p>
        </div>
      ...
      {/* Course cards */}
      {[
        { icon: "🌱", title: "Conceitos Básicos", desc: "Renda fixa vs variável, juros compostos, inflação, Selic. O essencial pra começar.", level: "Iniciante", color: C.green, tag: "Em breve" },
        { icon: "📊", title: "Análise de Investimentos", desc: "Indicadores fundamentalistas, como ler balanços, comparar empresas e FIIs.", level: "Intermediário", color: C.blue, tag: "Em breve" },
        { icon: "🧠", title: "Estratégias Avançadas", desc: "Cenários macro, marcação a mercado, hedge, diversificação global.", level: "Avançado", color: C.purple, tag: "Em breve" },
      ].map((c) => (
      ...
      <div style={{ textAlign: "center", padding: 20, marginTop: 12 }}>
        <p style={{ fontSize: 11, color: C.textMuted }}>Cursos, e-book e ferramentas em desenvolvimento.</p>
      </div>
    </div>
  );
}
